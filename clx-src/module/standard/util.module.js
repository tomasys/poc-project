...
ExcelUtil = {
	...
	_exportExcel: function(sheetData) {
		...
		if (!isIE) {
			a.onwriteend = function() {
				grid.getAppInstance().callAppMethod("hideMasks");
			}
		} else {
			if (a) {
				grid.getAppInstance().callAppMethod("hideMasks");
	...
	_setHdrStyle: function(poWs, paHdrMergeCols) {
		var voExportData = this._exportData;
		var voColLot = voExportData.cols;
		var vaHdrRange = [];
		...
		if (!paHdrMergeCols) {
			...
			vaHdrRange = paHdrMergeCols;
		}
		
		vaHdrRange.forEach(function(each, idx) {
			for (var R = each.s.r; R <= each.e.r; ++R) {
				for (var C = each.s.c; C <= each.e.c; ++C) {
					var cell_address = {
						c: C,
						r: R
					};
					var cell_ref = XLSX.utils.encode_cell(cell_address);
					if (poWs[cell_ref] == undefined) {
						return
					}
	...
	_setDtlStyle: function(poWs, paHdrMergeCols) {
		var voExportData = this._exportData;
		var voColLot = voExportData.cols;
		var vaHdrRange = [];
		
		if (!paHdrMergeCols) return;
	...
	_setColumnWidth: function(poWs) {
		
		var wsCols = [];
		var grid = this._grid;
		var exportData = this._exportData;
		var cols = exportData.cols;
		var regex = /[^0-9]/g;
		var headerText = exportData.rowgroups[0].data[0];
		
		cols.forEach(function(each, idx) {
			var vnWidth = each.width.replace(regex, "");			
			if(headerText[idx].indexOf("\r\n") != -1){
				vnWidth = Number(vnWidth)+70;
			}
			var width = {
				width: (Number(vnWidth) / 10)
			};
			wsCols.push(width);
		});
		poWs["!cols"] = wsCols;
	...
	_exportExcelToJson: function() {
		
		var ws;
		var _that = this;
		var vcGrid = _that._grid;
		var exportData = _that._exportData;
		...
		ws = XLSX.utils.json_to_sheet(resultData);
		
		/*컬럼너비 정의 */
		_that._setColumnWidth(ws);
		
		/*헤더 컬럼 스타일 정의 */
		_that._setHdrStyle(ws, false);
		
		/*데이터 셋으로 구성된 디테일 컬럼 String Center정렬*/
		if (vcGrid.type == "dataset") {
			var vaDtlRange = [];
			detail.style.forEach(function(each, idx) {
				if (each.type == "string") {
			...
			_that._setDtlStyle(ws, vaDtlRange);
		}
		
		/*엑셀 출력*/
		_that._exportExcel(ws);
	},
	
	_exportExcelToArray: function(pbisHasDtlMergeCols) {
		...
		var header, detail, footer;
		/** @type Array */
		var headerData, detailDatas, footerDatas;
		var headerStyle, detailStyle;
		
		voExportData.rowgroups.forEach(function(each) {
			if (each.region == "header") {
				header = each;
				headerData = each.data[0];
				headerStyle = each.style;
			} else if (each.region == "detail") {
				detail = each;
				detailDatas = each.data;
				detailStyle = each.style;
			} else if (each.region == "footer") {
				footer = each;
				footerDatas = each.data[0];
			}
		});
		
		var cols = voExportData.cols;
		
		// 헤더 데이터 구성
		var maxHdrRowIdx = vcGrid.header.getRowHeights().length - 1;
		for (var i = 0; i <= maxHdrRowIdx; i++) {
			var hdrRowData = [];
			var rowMergeInfo = [];
			headerStyle.forEach(function(each) {
				var cellIdx = each.cellIndex;
				if (each.rowindex == i) {
					for (var j = 0; j < each.colspan; j++) hdrRowData.push(headerData[cellIdx]);
				} else {
					if (each.rowspan > 1) rowMergeInfo.push(each);
				}
			});
			
			rowMergeInfo.forEach(function(each) {
				for (var k = 0; k < each.colspan; k++) {
					hdrRowData.splice(each.colindex, 0, headerData[each.cellIndex]);
				}
			});
			
			resultData.push(hdrRowData);
		}
		...
		headerStyle.forEach(function(each) {
			var mergesInfo = {
				...
				e: {
					c: each.colindex + each.colspan - 1,
					r: each.rowindex + each.rowspan - 1
				}
			}
			
			if (each.colspan > 1 || each.rowspan > 1) {
				wsMerges.push(mergesInfo);
				wsHdrStyleRange.push(mergesInfo);
			} else {
				wsHdrStyleRange.push(mergesInfo);
			}
		});
		
		if (!pbisHasDtlMergeCols) { // 디테일 행이 병합, 다중 로우가 아닐경우
			detailDatas.forEach(function(eachs) {
				resultData.push(eachs);
			});
		} else {
			
			// 디테일 데이터 구성
			var maxDtlRowIdx = vcGrid.detail.getRowHeights().length - 1;
			var maxColIdx = 0;
			var maxRowColCnt = 0;
			var vaMergeColInfo = [];
			
			detailStyle.forEach(function(each) {
				if (each.colindex > maxColIdx) {
					maxColIdx = each.colindex;
					maxRowColCnt++;
				}
				if (each.colspan > 1 || each.rowspan > 1) vaMergeColInfo.push(each);
			});
			
			detailDatas.forEach(function(datas, idx) {
				var dtlRowData = [];
				var s = 0;
				var e = maxRowColCnt;
				
				/* 디테일 데이터 처리*/
				for (var i = 0; i <= maxDtlRowIdx; i++) {
					dtlRowData = datas.slice(s, e + 1);
					
					vaMergeColInfo.forEach(function(each) {
						var curentMaxLength = resultData.length;
						if (each.rowspan > 1) {
							if (each.rowindex < i && each.rowindex < each.rowindex + each.rowspan - 1) {
								dtlRowData.splice(each.colindex, 0, "");
							} else if (each.rowindex == i) {
								wsMerges.push({
									s: {
										c: each.colindex,
										r: curentMaxLength
									...
									e: {
										c: each.colindex + each.colspan - 1,
										r: curentMaxLength + each.rowspan - 1
									}
								});
							}
							
						} else if (each.colspan > 1) {
							if (each.rowindex == i) {
								dtlRowData.splice(each.colindex + each.colspan - 1, 0, "");
		...
		if (footer) { // 푸터 존재시 생성
			var colLot = vcGrid.getColumnLayout();
			var colLotFooter = colLot.footer;
			var footerMerges = [];
			var mergeRowIdx = resultData.length;
			
			for (var i = 0; i < vcGrid.footer.getRowHeights().length; i++) {
				var footerData = [];
				var rowIdxs = colLotFooter.filter(function(each) {
					return each.rowIndex == i;
				});
				
				var clotLength = colLot.columnLayout.length;
				for (var j = 0; j < clotLength; j++) {
					var filter = rowIdxs.filter(function(each) {
						return each.colIndex == j;
					})
					
					if (filter.length > 0) {
						footerData.push(footerDatas[filter[0].cellIndex]);
					} else {
						footerData.push("");
					}
				}
				resultData.push(footerData);
			}
			
			colLotFooter.forEach(function(each) {
				if (each.rowSpan > 1 || each.colSpan > 1) {
					footerMerges.push({
						colIndex: each.colIndex,
			...
			footerMerges.forEach(function(each) {
				var merges = {
					s: {},
					e: {}
				};
				...
				if (each.rowSpan > 1) {
					merges["s"]["r"] = mergeRowIdx;
					merges["e"]["r"] = mergeRowIdx + each.rowSpan - 1;
					
				} else {
					merges["s"]["r"] = mergeRowIdx;
					merges["e"]["r"] = mergeRowIdx;
				}
				
				if (each.colSpan > 1) {
					merges["s"]["c"] = each.colIndex;
					merges["e"]["c"] = each.colIndex + each.colSpan - 1;
				} else {
					merges["s"]["c"] = each.colIndex;
					merges["e"]["c"] = each.colIndex;
				}
				wsMerges.push(merges);
			});
		}
		
		ws = XLSX.utils.aoa_to_sheet(resultData);
		ws["!merges"] = wsMerges;
		
		this._setHdrStyle(ws, wsHdrStyleRange);
		
		this._setColumnWidth(ws);
		
		this._exportExcel(ws);
	},
	
	/**
	 * 데이터 셋내 데이터 를 ExportEata로 반환합니다.
	 * @param {cpr.data.DataSet} dataSet 데이터 셋
	 * @param {Number} colWidth 출력될 컬럼 너비
	 */
	getDsExportData: function(dataSet, colWidth, pbIsColInfo) {
		/** @type cpr.data.DataSet */
		var vcDs = dataSet;
		colWidth = ValueUtil.isNull(colWidth) ? "150px" : colWidth;
		colWidth = colWidth.indexOf("px") != -1 ? colWidth : colWidth + "px";
		var exportData = cpr.utils.ExportUtil.getExportData(vcDs);
		
		for (var i = 0; i < exportData.rowgroups[0].data[0].length; i++) {
			
			if (pbIsColInfo) {
				var colInfo = vcDs.getHeaders()[i].getInfo();
				if (!ValueUtil.isNull(colInfo)) {
					exportData.rowgroups[0].data[0][i] = colInfo;
				}
			}
			
			// 컬럼 width 정의
			var hdrColNm = exportData.rowgroups[0].data[0][i];
			if (hdrColNm.length > 5) {
				var width = (hdrColNm.length * 25 + 20) + "px";
				exportData.cols[i] = {
					width: width
				};
			} else {
	 ...
	 * @param {any} targetCtrl 그리드 컨트롤 또는 데이터 셋
	 * @param {any} paExcludeCols? 예외 컬럼 인덱스
	 * @param {any} pbIsColInfo? 컬럼명을 colInfo로 사용할지 여부
	 */
	exportExcel: function(fileName, sheetName, targetCtrl, paExcludeCols, pbIsColInfo) {
		
		var that = this;
		
		var voResourceLoader = new cpr.core.ResourceLoader();
		voResourceLoader.addScript("./thirdparty/excel/xlsx.min.js");
		voResourceLoader.addScript("./thirdparty/excel/shim.min.js"); 
		voResourceLoader.addScript("./thirdparty/excel/FileSaver.min.js");
		if (voResourceLoader.needToLoad()) {
			voResourceLoader.load().then(function(input) {
				cpr.core.ResourceLoader.loadScript("./thirdparty/excel/xlsx.bundle.js").then(function(input) {
					that.exportExcel(fileName, sheetName, targetCtrl, paExcludeCols, pbIsColInfo);
				});
			});
			return false;
		}
...
//		if (voResourceLoader.needToLoad()) {
//			voResourceLoader.load().then(function(input) {
//				that.exportExcel(fileName, sheetName, targetCtrl, paExcludeCols);
//			});
//			return false;
//		}
		
		this._fileName = fileName.indexOf(".xlsx") != -1 ? fileName : fileName + ".xlsx";
		this._sheetName = ValueUtil.isNull(sheetName) ? "sheet1" : sheetName;
		this._grid = targetCtrl;
		
		var vcTarget = targetCtrl;
		var vaExcludeCols = [];
		
		// export 제외컬럼 설정
		if (paExcludeCols != null) {
			if (!(paExcludeCols instanceof Array)) paExcludeCols = [paExcludeCols];
			vaExcludeCols = paExcludeCols;
		}
		
		var exportData = {};
		
		if (vcTarget instanceof cpr.controls.Grid) {
			exportData = vcTarget.getExportData({
				exceptStyle: true,
				applyFormat: true,
				excludeCellIndex: vaExcludeCols
			});
		} else if (vcTarget instanceof cpr.data.DataSet) {
			exportData = this.getDsExportData(vcTarget, null, pbIsColInfo);
		}
		
		this._exportData = exportData;
		
		var isHasHdrMergeCols = false;
		var isHasDtlMergeCols = false;
		...
		/** @type Array */
		var detailStyle = detail.style;
		
		// 헤더 및 디테일 행 로우수 및 병합 여부 확인
		headerStyle.forEach(function(each) { // 헤더
			if (each.rowindex > 0 || each.rowspan > 1 || each.colspan > 1) {
				isHasHdrMergeCols = true;
				return false;
			}
		});
		
		detailStyle.forEach(function(each) { // 디테일
			if (each.rowindex > 0 || each.rowspan > 1 || each.colspan > 1) {
				isHasDtlMergeCols = true;
				return false;
			}
		});
		...
		// 푸터 존재 여부
		var hasFooter = false;
		if (vcTarget instanceof cpr.controls.Grid) hasFooter = vcTarget.hasFooter();
		
		if (isHasHdrMergeCols || isHasDtlMergeCols || hasFooter) {
			this._exportExcelToArray(isHasDtlMergeCols);
		} else {
			this._exportExcelToJson();
		}
	},
	 ...
	 */
	importExcel: function(poFile, pcGrid, poCallbackFunc) {
		var that = this;
		var voResourceLoader = new cpr.core.ResourceLoader();
		voResourceLoader.addScript("./thirdparty/excel/xlsx.min.js");
		voResourceLoader.addScript("./thirdparty/excel/FileSaver.min.js");
		if (voResourceLoader.needToLoad()) {
			voResourceLoader.load().then(function(input) {
				cpr.core.ResourceLoader.loadScript("./thirdparty/excel/xlsx.bundle.js").then(function(input) {
					that.importExcel(poFile, pcGrid, poCallbackFunc);
				});
			});
			return false;
		}
		...
		reader.onload = function(e) {
			...
			workbook.SheetNames.forEach(function(item, index) {
				...
				range.s.r = 1;
				// 데이터셋에서 컬럼이름 가져오기
				var _header = pcGrid.dataSet.getColumnNames();
				var EXCEL_JSON;
