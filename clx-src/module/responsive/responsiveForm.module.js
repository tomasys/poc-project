	...
	this._container.addEventListener("before-draw", function(e){
		cpr.core.DeferredUpdateManager.INSTANCE.asyncExec(function(){
			...
			if(!isEqual) {
				...
				that._onScreenChange(new cpr.events.CScreenChangeEvent({name: that._appInstance.targetScreen.name}));
			}
		});
	});
...
RForm.prototype._transform = function(colCount) {
	...
	for (var idx = 0; idx < visibleChildren.length; idx++) {
		...
		if(ValueUtil.isStartWith(each.id, AppProperties.SEARCH_BTN_ID)) {
			// 조회버튼 식별 로직
			// AIGen 으로 동적 구획이 추가된 후 반응형이 동작할 수 있도록 asyncExec 추가
			var vcTempButton = each;
			cpr.core.DeferredUpdateManager.INSTANCE.asyncExec(function() {
				that._container.replaceConstraint(vcTempButton, {
					rowIndex: rowIndex,
...
//		});
		...
		|| this._screenNms["tablet"].indexOf(that._container.getAppInstance().targetScreen.name) != -1 && that._container.userAttr(that.ATTR_NM.ATTR_TABLET_COLUMN_COUNT) == 1) {
			if (!that._container.style.hasClass("search-box") && visibleChildren[idx] instanceof cpr.controls.Output && _.contains(visibleChildren[idx].style.getClasses(), msLabelClass)) {
